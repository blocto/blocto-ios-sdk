...
struct BTFunction: Equatable, CustomStringConvertible {
    public var name: String
    public var parameters: [BTABIType]

    public init(name: String, parameters: [BTABIType]) {
        self.name = name
    ...
    public func castArguments(_ values: [Any]) throws -> [BTABIValue] {
        if values.count != parameters.count {
            throw BTABIError.invalidNumberOfArguments
        }
        return try zip(parameters, values).map({ try BTABIValue($1, type: $0) })
    }
