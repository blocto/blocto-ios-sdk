...
indirect enum BTABIValue: Equatable {
    ...
    /// A function call
    case function(BTFunction, [BTABIValue])

    /// Fixed-length array where all values have the same type
    case array(BTABIType, [BTABIValue])
    ...
    case dynamicArray(BTABIType, [BTABIValue])

    /// Tuple
    case tuple([BTABIValue])

    /// Value type
    public var type: BTABIType {
        switch self {
    ...
    public init(_ value: Any, type: BTABIType) throws {
        switch (type, value) {
        ...
        case (.address, let address as AnyAddress):
        ...
        case (.array(let type, _), let array as [Any]):
            self = .array(type, try array.map({ try BTABIValue($0, type: type) }))
        ...
        case (.dynamicArray(let type), let array as [Any]):
            self = .dynamicArray(type, try array.map({ try BTABIValue($0, type: type) }))
        case (.tuple(let types), let array as [Any]):
            self = .tuple(try zip(types, array).map({ try BTABIValue($1, type: $0) }))
        default:
