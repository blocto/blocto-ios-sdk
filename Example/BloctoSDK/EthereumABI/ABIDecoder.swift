...
final class BTABIDecoder {
    let data: Data
    var offset = 0

    /// Creates an `BTABIDecoder`.
    public init(data: Data) {
        self.data = data
    }

    /// Decodes an `BTABIValue`
    public func decode(type: BTABIType) throws -> BTABIValue {
        switch type {
        case .uint(let bits):
    ...
    public func decodeArray(type: BTABIType) throws -> [BTABIValue] {
        let count = Int(decodeUInt())
        return try decodeArray(type: type, count: count)
    }

    /// Decodes a static array
    public func decodeArray(type: BTABIType, count: Int) throws -> [BTABIValue] {
        return try decodeTuple(types: Array(repeating: type, count: count))
    }

    /// Decodes a tuple
    public func decodeTuple(types: [BTABIType]) throws -> [BTABIValue] {
        let baseOffset = offset

        var values = [BTABIValue]()
        for subtype in types {
            let value: BTABIValue
    ...
    public func decode(function: BTFunction) throws -> BTABIValue {
        if try BTABIEncoder.encode(signature: function.description) != decodeSignature() {
            throw BTABIError.functionSignatureMismatch
        }
    ...
    public func decodeUInt() -> BigUInt {
        assert(offset + BTABIEncoder.encodedIntSize <= data.count)
        let value = BigUInt(data.subdata(in: offset ..< offset + BTABIEncoder.encodedIntSize))
        offset += BTABIEncoder.encodedIntSize
        return value
    }
    ...
    public func decodeInt() -> BigInt {
        ...
        let max = BigInt(1) << (BTABIEncoder.encodedIntSize * 8)
    ...
    /// Decodes an address
    public func decodeAddress() -> AnyAddress {
        let addressLength = 20
        let ethereumAddress = AnyAddress(string: data.subdata(in: offset + BTABIEncoder.encodedIntSize - addressLength ..< offset + BTABIEncoder.encodedIntSize).hexString, coin: .ethereum) ?? .ethZero
        offset += BTABIEncoder.encodedIntSize
    ...
    public func decodeString() throws -> String {
        ...
        guard let string = String(data: data.subdata(in: offset ..< offset + count), encoding: .utf8) else {
            throw BTABIError.invalidUTF8String
        }
        offset += count
        return string
    }
